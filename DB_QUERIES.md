...
- **`Back/app/services/admin_service.py`**
  - 서브쿼리 (최신 체크인/체크아웃):
    - line 134: `(SELECT checkin_time FROM checkins WHERE member_id = m.member_id ORDER BY checkin_time DESC LIMIT 1) as last_checkin_time`
    - line 135: `(SELECT checkout_time FROM checkins WHERE member_id = m.member_id ORDER BY checkin_time DESC LIMIT 1) as last_checkout_time`
    - line 171, 173, 254, 256: 체크인 상태 판단/카운트에서 사용된 유사 서브쿼리 (`... LIMIT 1`)들
  - JOIN:
    - line 438: `JOIN members m ON c.member_id = m.member_id`

- **`Back/app/repositories/checkin_repository.py`**
  - JOIN / LEFT JOIN (회원 정보 결합):
    - line 46: `JOIN members m ON m.member_id = c.member_id`
    - line 60: `JOIN members m ON m.member_id = c.member_id`
    - line 142: `LEFT JOIN members m ON c.member_id = m.member_id`
  - 이벤트 생성 관련 (`CREATE EVENT`):
    - line 41, 55 등: per-checkin `CREATE EVENT` 빌드/삽입 로직
  - `LIMIT 1` 사용 (단건 조회/페이징 등): line 104, 117

- **`Back/app/auto_delete_triggers.py`**
  - JOIN (자동 정리용 UPDATE/SELECT):
    - line 63: `JOIN members m ON m.member_id = c.member_id`
  - 트리거 생성/제거:
    - drop trigger: line 86 (`DROP TRIGGER IF EXISTS before_member_delete`)
    - drop trigger: line 107, 108 (`before_checkin_insert_immediate`, `after_checkin_insert_immediate` 제거)
    - create trigger: line 111 (`CREATE TRIGGER before_checkin_insert_immediate`)
    - create trigger: line 123 (`CREATE TRIGGER after_checkin_insert_immediate`)
  - 이벤트 생성:
    - line 149: `CREATE EVENT auto_delete_old_members`

- **`Back/app/repositories/member_repository.py`**
  - JOIN (간접 언급 / 체크인 결합):
    - line 395: `JOIN members m ON cl.member_id = m.member_id`
  - `ON DUPLICATE KEY UPDATE` 사용 예:
    - line 243: `ON DUPLICATE KEY UPDATE deleted_at = NOW()` (soft-delete/복원 관련)

- **`Back/app/repositories/deleted_member_repository.py`**
  - `ON DUPLICATE KEY UPDATE` 사용:
    - line 79, line 185: `ON DUPLICATE KEY UPDATE` (복원/삽입시 중복 처리)
  - 영구삭제/복원 로직(파일 전반에 걸쳐 `INSERT`, `DELETE` 호출 존재)

- **`Back/app/repositories/admin_repository.py`**
  - 단건 조회 예시:
    - line 8: `SELECT * FROM admins LIMIT 1`

**요약(자동 수집 기준)**
- SQL `JOIN members` 관련 명시적 사용: 6건 (주로 `checkin_repository.py`, `admin_service.py`, `member_repository.py`, `auto_delete_triggers.py`)
- `LIMIT 1` 포함(서브쿼리/단건 조회) 사용: 9건 (주로 `admin_service.py`, `checkin_repository.py`, `admin_repository.py`)
- `CREATE EVENT`: 3~4건 (per-checkin 이벤트 빌드 및 자동 삭제 이벤트)
- `TRIGGER` 관련 구문: 다수(생성/삭제 포함) — `auto_delete_triggers.py`에 집중

위 항목은 레포 파일들에서 자동 검색한 결과를 기반으로 합니다. 원하시면 다음을 추가로 진행하겠습니다:
- 각 항목의 정확한 코드 라인 전체(주변 컨텍스트 포함) 삽입
- 각 쿼리별 호출 횟수(런타임에서의 실제 호출 빈도) 분석을 위한 로그/테스트 코드 추가 제안
- `DB_QUERIES.md` 내 기존 설명과 병합(중복 제거/구조 개선)

**총계 (정적 검색 기준, `Back/` 폴더)**

- **서브쿼리 (`(SELECT ...)`)**: **6건**
  - 모두 `Back/app/services/admin_service.py`에서 발견
    - lines: 134, 135, 171, 173, 254, 256

- **JOIN / LEFT JOIN**: **6건**
  - `Back/app/services/admin_service.py` — line 438: `JOIN members m ON c.member_id = m.member_id`
  - `Back/app/repositories/member_repository.py` — line 395: `JOIN members m ON cl.member_id = m.member_id`
  - `Back/app/repositories/checkin_repository.py` — lines 46, 60: `JOIN members m ON m.member_id = c.member_id`
  - `Back/app/repositories/checkin_repository.py` — line 142: `LEFT JOIN members m ON c.member_id = m.member_id`
  - `Back/app/auto_delete_triggers.py` — line 63: `JOIN members m ON m.member_id = c.member_id`

- **TRIGGER 관련 키워드**: **7건** (생성/제거 포함)
  - `Back/app/auto_delete_triggers.py` — line 86: `DROP TRIGGER IF EXISTS before_member_delete`
  - `Back/app/auto_delete_triggers.py` — line 107: `DROP TRIGGER IF EXISTS before_checkin_insert_immediate`
  - `Back/app/auto_delete_triggers.py` — line 108: `DROP TRIGGER IF EXISTS after_checkin_insert_immediate`
  - `Back/app/auto_delete_triggers.py` — line 111: `CREATE TRIGGER before_checkin_insert_immediate`
  - `Back/app/auto_delete_triggers.py` — line 123: `CREATE TRIGGER after_checkin_insert_immediate`
  - `Back/app/auto_delete_triggers.py` — line 104 and line 208: 주석/관련 설명 포함(총 키워드 등장 수에 포함)

참고: 위 총계는 워크스페이스 내 정적 문자열 검색 결과를 집계한 값입니다. 런타임 호출 빈도나 동적으로 생성되는 쿼리는 포함되지 않습니다. 더 정확한 런타임 계측이 필요하면 로그 계측 또는 테스트 스크립트를 추가하는 것을 권장합니다.
